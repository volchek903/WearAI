...
from app.states.feb23_flow import Feb23Flow
...
_PROMPT = (
    "Create a 9:16 portrait without changing facial features. "
    "In the image, a couple stands against a foggy forest or a ruined landscape. "
    "Only their faces are in the frame. The man and woman are in modern, neutral tactical-style "
    "uniforms with no national symbols, flags, insignia, or country-specific emblems. "
    "The man wears a tactical helmet. The woman has loose hair, wind blowing through it. "
    "They look emotionally engaged. They are close, their faces almost touching, creating an intimate mood. "
    "Both have visible traces of dirt and dust on their faces, hinting at hardship. "
    "The woman looks down, the man slightly tilts his head toward her; their gazes meet. "
    "Handwritten text at the top: «С праздником!». "
    "Below in large letters: «Мой защитник» and below that: «моя гордость». "
    "Background slightly blurred, dark to enhance drama, highlighting the couple. "
    "Soft lighting with gentle shadows for depth and sensitivity."
)
...
@router.callback_query(F.data == MenuCallbacks.FEB23)
async def start_feb23(
...
async def feb23_photos_in(message: Message, state: FSMContext) -> None:
...
@router.callback_query(Feb23Flow.confirm, F.data == ConfirmCallbacks.NO)
async def feb23_confirm_no(call: CallbackQuery, state: FSMContext) -> None:
...
@router.callback_query(Feb23Flow.confirm, F.data == ConfirmCallbacks.YES)
async def feb23_confirm_yes(
    call: CallbackQuery, state: FSMContext, session: AsyncSession
) -> None:
    ...
    if not photos:
        await state.clear()
        await call.answer("Фото не найдены 😕", show_alert=True)
        return
    ...
    try:
        ...
        logger.warning("FEB23 generation failed: %s", e)
        if not sent_any:
        ...
        logger.exception("FEB23 generation failed: %s", e)
        if not sent_any:
            await refund_photo_generation(session, tg_id)
        await stop_progress(stop, progress_task)
        await edit_text_safe(
