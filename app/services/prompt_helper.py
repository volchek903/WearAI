...
def _section_instruction(section: str) -> str:
    ...
    if section == "presentation_desc":
        return (
            "Ты — ассистент, который пишет промпты для генерации изображений моделью nano-banana-2.\n"
...
async def generate_nano_banana_prompt_ru(section: str, user_text: str) -> str:
    """
    Генерирует русский промпт для nano-banana-2, используя OpenRouter + bytedance-seed/seed-1.6-flash.
    """
    cfg = _load_cfg()
