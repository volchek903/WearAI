...
from app.states.march8_flow import March8Flow
...
def _prompt_with_text(text: str) -> str:
    ...
    return (
        "Акварельный портрет. Взрослый ребёнок (дочь или сын) нежно обнимает маму. "
        "Они одеты в нежно-пастельные блузки или рубашки. Мама с любовью "
        "прижимает к себе большой букет розовых тюльпанов. "
        f"Внизу изображения добавлена надпись «{safe_text}». "
        "Освещение мягкое и естественное, подчеркивающее невинность и теплоту сцены. "
        "Композиция — вертикальный крупный план. Размер 9:16. "
        "Лица срисованы по исходным фотографиям, без изменения черт."
    )
...
@router.callback_query(F.data == MenuCallbacks.MARCH8)
async def start_march8(
...
@router.message(March8Flow.photos)
async def march8_photos_in(message: Message, state: FSMContext) -> None:
    ...
    if not message.media_group_id:
        ...
        await state.set_state(March8Flow.text)
...
@router.message(March8Flow.text)
async def march8_text_in(message: Message, state: FSMContext) -> None:
    ...
    await state.set_state(March8Flow.confirm)

    if len(photos) == 1:
        await message.answer_photo(
            photos[0],
            caption="Проверь, всё ли верно 👇",
        )
    else:
...
@router.callback_query(March8Flow.confirm, F.data == ConfirmCallbacks.NO)
async def march8_confirm_no(call: CallbackQuery, state: FSMContext) -> None:
    await state.update_data(photos=[], text="")
    await state.set_state(March8Flow.photos)
...
@router.callback_query(March8Flow.confirm, F.data == ConfirmCallbacks.YES)
async def march8_confirm_yes(
    call: CallbackQuery, state: FSMContext, session: AsyncSession
) -> None:
    data = await state.get_data()
    ...
    try:
        ...
        logger.warning("MARCH8 generation failed: %s", e)
        if not sent_any:
        ...
        logger.exception("MARCH8 generation failed: %s", e)
        if not sent_any:
            await refund_photo_generation(session, tg_id)
        await stop_progress(stop, progress_task)
        await edit_text_safe(
